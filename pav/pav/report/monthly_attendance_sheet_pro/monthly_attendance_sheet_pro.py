...
def execute(filters=None):
	...
	att_maph = get_wh(conditions, filters)
	...
	leave_list = None
	if filters.summarized_view and not filters.summarized_hours_view:
	...
	if filters.group_by:
		...
		for parameter in group_by_parameters:
			...
			record, aaa = add_data(emp_map[parameter], att_map, att_maph, filters, holiday_map, conditions, default_holiday_list, leave_list=leave_list)
			emp_att_map.update(aaa)
			data += record
	else:
		record, emp_att_map = add_data(emp_map, att_map, att_maph, filters, holiday_map, conditions, default_holiday_list, leave_list=leave_list)
		data += record
...
def add_data(employee_map, att_map, att_maph, filters, holiday_map, conditions, default_holiday_list, leave_list=None):

	record = []
	emp_att_map = {}
	for emp in employee_map:
		emp_det = employee_map.get(emp)
		if not emp_det or emp not in att_map or emp not in att_maph:
			continue
		...
		total_wh = total_p = total_a = total_l = total_h = total_um= 0.0
		emp_status_map = []
		wkh = []
		for day in range(filters["total_days_in_month"]):
			status = None
			status = att_map.get(emp).get(day + 1)
			working_hours = flt(att_maph.get(emp).get(day + 1))
			...
			if  filters.summarized_hours_view and not filters.summarized_view:
				if working_hours:
					total_wh += working_hours

		if not filters.summarized_view and not filters.summarized_hours_view:
			row += emp_status_map
		...
		if filters.summarized_hours_view and not filters.summarized_view:
			wh_counts = frappe.db.sql("""select (select sum(working_hours) from `tabAttendance` where \
				docstatus = 1 %s) as work_h_count""" % conditions , filters)

			row.extend([wh_counts[0][0]])
...
def get_columns(filters):
	...
	if filters.summarized_hours_view and not filters.summarized_view:
		columns += [_("Total Working Hours") + ":Float:120"]
	return columns, days

def get_attendance_list(conditions, filters):
	attendance_list = frappe.db.sql("""select employee, day(attendance_date) as day_of_month,
		status, working_hours from tabAttendance where docstatus = 1 %s order by employee, attendance_date""" %
...
def get_wh(conditions, filters):
	...
	att_maph = {}
	for d in attendance_list:
		att_maph.setdefault(d.employee, frappe._dict()).setdefault(d.day_of_month, "")
		att_maph[d.employee][d.day_of_month] = d.working_hours
