...
def execute(filters=None):
	...
	if not filters.get("project_activity"): return [], []
...
def get_conditions(filters):
	...
	employees = frappe.db.sql("""select employee from `tabProject Activity Payroll` where
		parent = '%s' and status = 'Active' """ % filters.get("project_activity"), as_dict=1) if filters.get("project_activity") else None

	if employees:
		frappe.msgprint("employees=== {0} ".format(employees))
		conditions += " and employee in %(employees)s"
		filters.update({"employees": tuple([d.employee for d in employees])})
